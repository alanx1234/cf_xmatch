...
def _compute_loss(
    ...
    Per-star log_prob = mean over n_samples; then outlier-mixture loss averaged over batch.
    """
    x_exp     = x_b.repeat_interleave(n_samples, dim=0)          # (B*n_samples, 1)
    lp        = flow(c_exp).log_prob(x_exp)                       # (B*n_samples,)
    lp_star   = lp.reshape(-1, n_samples).mean(dim=1)             # (B,)
...
def train_fold(
    ...
    # Pre-draw fixed age samples once per fold — same 10 samples used across all epochs
    N = len(x_train)
    ...
    for epoch in range(epochs):
        ...
        for x_b, idx_b in train_loader:
            ...
            loss    = _compute_loss(flow, x_b, c_exp_b, n_samples, ln_p_out)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            if use_onecycle:
        ...
        if has_val:
            flow.eval()
            with torch.no_grad():
                val_loss = _compute_loss(flow, x_val, c_val_exp, n_samples, ln_p_out)
